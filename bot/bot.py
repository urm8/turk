...
from data_base.sqlite_db import (
    ...
    answer_table_read_entry,
    sql_start,
    stats_table_check_entry_existence,
    stats_table_create_entry,
    stats_table_decrease_stats,
    stats_table_increase_stats,
...
async def quiz_start(message: types.Message):
    ...
    if user_table_check_existence(message.chat.id):
        ...
        if n_of_words < 2:
            ...
            answer_table_add_entry(
                message.chat.id, current_poll.poll.id, correct_option_id, new_word,
            )
    else:
...
async def handle_poll_answer(quiz_answer: PollAnswer):
    ...
    if user_table_check_existence(quiz_answer.user.id):
        if answer_table_check_entry_existence(quiz_answer.poll_id):
            user_id, poll_id, correct_option_id, current_word = answer_table_read_entry(
                quiz_answer.poll_id,
            )
            ...
            if n_of_words < 2:
                ...
                answer_table_add_entry(
                    quiz_answer.user.id, current_poll.poll.id, correct_option_id, new_word,
                )
        else:
