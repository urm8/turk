...
def answer_table_add_entry(user_id, poll_id, correct_option_id: int, word: str):
    cur.execute(
        ...
        (user_id, poll_id, correct_option_id, word),
    )
    base.commit()
...
def user_table_create(user_id: int):
    ...
    base.execute(
        f'CREATE TABLE IF NOT EXISTS {table_name} (word TEXT PRIMARY KEY, description TEXT)',
    )
    base.commit()
...
def user_table_add_default_entries(user_id: int):
    ...
    if cur.execute(f'SELECT COUNT(*) FROM {table_name}').fetchone()[0] == 0:
        cur.execute(
            f'INSERT INTO {table_name} VALUES ("bir", "one"), ("iki", "two"), ("üç", "three"),'
            f'("orada", "there")',
        )
        base.commit()
...
def user_table_check_entry_existence(word: str, user_id: int):
    table_name = 'table_' + str(user_id)
    return cur.execute(
        f'SELECT EXISTS(SELECT 1 FROM {table_name} WHERE LOWER(word) = LOWER(?))',
        (word,),
    ).fetchone()[0] == 1
...
def user_table_delete_entry(word_to_remove: str, user_id: int):
    table_name = 'table_' + str(user_id)
    cur.execute(
        f'DELETE FROM {table_name} WHERE LOWER(word) = LOWER(?)', (
            word_to_remove,
        ),
    )
    base.commit()
...
def stats_table_increase_stats(user_id: int):
    cur.execute(
        'UPDATE stat_table SET correct = correct + 1 WHERE user_id = ?', (
            user_id,
        ),
    )
    base.commit()
...
def stats_table_decrease_stats(user_id: int):
    cur.execute(
        'UPDATE stat_table SET incorrect = incorrect + 1 WHERE user_id = ?', (
            user_id,
        ),
    )
    base.commit()
